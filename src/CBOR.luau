...
--[[
This is lua-cbor created by Kim Alvefur aka Zash
for Lua, not Luau.
The original can be found at https://github.com/Zash/lua-cbor
Slightly edited by me to get it functioning in luau.
]]
...
local function encode(obj, opts)
	if type(obj) == "userdata" or type(obj) == "vector" then
		return error("Cannot encode {userdata | vector} inside of table.")
	end
...
function encoder.userdata(ud, opts)
	...
	error("can't encode "..typeof(ud).." into a cbor table",5)
end
